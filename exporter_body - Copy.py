...
def export_body(exportfolderpath, bodyNo) :
	...
	for poly in me.polygons:
		...
		for i in range(len(poly.vertices)):
			...
			vertindex = me.vertices[poly.vertices[i]].index
			edgekey = edgekeys[i]
			if not edgekey in edgepointkeys:
				print(vertindex, edgekey)
			else:
				index = edgepointkeys.index(edgekey)
...
for i, w in get_weights(ob, "root"):
    print(i, w)

weights = list(get_weights(ob, ob.vertex_groups['root']))
print(weights)
	...
	fileout.write( " , ".join(pyBlock_MeshData_BlendControl) )
	fileout.flush()
	fileout.close()
