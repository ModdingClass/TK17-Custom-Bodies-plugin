...
def export_arms_fix(exportfolderpath, bodyNo):
	...
	snippet = snippet+ "\t.RotationPivot ("        +" {:.6f}f,".format(wrist_L_joint.location.y+ 0)    +" {:.6f}f,".format(wrist_L_joint.location.x+ 0)    +" {:.6f}f".format(wrist_L_joint.location.z+ 0)   +" );\n"  #here we switch  from yzx to yxz	
	snippet = snippet+ "\t.Translation ("        +" {:.6f}f,".format(forearm_L_joint.location.y+ 0)    +" {:.6f}f,".format(forearm_L_joint.location.z+ 0)    +" {:.6f}f".format(forearm_L_joint.location.x+ 0)   +" );\n"  #no switch
	snippet = snippet+ "};\n"
	snippet = snippet+""
	arms_fix_string = arms_fix_string+snippet
	arms_fix_string=arms_fix_string + "\n"
	...
	calculated_elbow_L_group_world_coordinates = mathutils.geometry.intersect_line_plane(line_a, line_b, plane_co, plane_no)
	...
	snippet = snippet+ "\t.RotationPivotTranslation ("        +" {:.6f}f,".format(0)    +" {:.6f}f,".format(0)    +" {:.6g}f".format(0)   +" );\n"   #here we switch to xz -y	
	snippet = snippet+ "};\n"
	snippet = snippet+""
	arms_fix_string = arms_fix_string+snippet
	arms_fix_string=arms_fix_string + "\n"	
	...
	plane_no = Vector ((-1,0,0))
	calculated_elbow_R_group_world_coordinates = mathutils.geometry.intersect_line_plane(line_a, line_b, plane_co, plane_no)
