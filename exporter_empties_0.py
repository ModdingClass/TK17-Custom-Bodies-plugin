...
def export_joints_fix(exportfolderpath, bodyNo):
	...
	for joint in animSkeletonValues:
		if joint!="root":
			...
			snippet = snippet+ "\t.JointOrientation ("   +" {:.6g}f,".format(degrees(empty.rotation_euler.y)+ 0)    +" {:.6g}f,".format(degrees(empty.rotation_euler.z)+ 0)    +" {:.6g}f".format(degrees(empty.rotation_euler.x)+ 0)   +" );\n"
			snippet = snippet+ "\t.Translation ("        +" {:.6g}f,".format(empty.location.y+ 0)    +" {:.6g}f,".format(empty.location.z+ 0)    +" {:.6g}f".format(empty.location.x+ 0)   +" );\n"	
